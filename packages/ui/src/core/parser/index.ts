...
export async function parseStreamedMessages({
  ...
  for await (const chunk of chunks) {
    ...
    // Handle custom metadata chunks from our API (chatId, done)
    if ((chunk as any).chatId && (chunk as any).done) {
      // Dispatch event with chatId for new chats
      window.dispatchEvent(
        new CustomEvent('chat-created', {
          detail: { chatId: (chunk as any).chatId },
        }),
      );
      continue;
    }
