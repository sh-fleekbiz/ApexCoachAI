...
export class ChatComponent extends LitElement {
  ...
  @property({ type: String, attribute: 'data-chat-id' })
  chatId: string | undefined = undefined;

  @property({ type: String, attribute: 'data-personality-id' })
  personalityId: string | undefined = undefined;
  ...
  loadMessages(messages: Array<{ role: string; content: string; citations?: any[] }>): void {
    this.chatThread = messages.map((msg) => {
      const timestamp = new Date().toISOString(); // Use current time or parse from message if available
      return {
    ...
    this.isChatStarted = this.chatThread.length > 0;
    this.requestUpdate();
  }
