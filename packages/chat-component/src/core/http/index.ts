...
export async function callHttpApi(
  ...
  { method, url, stream, signal, chatId, personalityId }: ExtendedChatHttpOptions,
) {
  // Use new API format for chat when chatId or personalityId is provided
  if (type === 'chat' && (chatId !== undefined || personalityId !== undefined)) {
    // Parse chatId and personalityId to numbers if provided
    const parsedChatId = chatId ? (typeof chatId === 'string' ? Number.parseInt(chatId, 10) : chatId) : null;
    ...
    const apiUrl = baseUrl.endsWith('/api/chat') ? baseUrl : `${baseUrl}/api/chat`;
